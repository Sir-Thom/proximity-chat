    ...
    try {
      const locationref =  firebase.database().ref("locations");
       ...
       if (firebase.auth().currentUser.uid) {
            ...
            const snapshot = await locationref.orderByChild('userid').equalTo(firebase.auth().currentUser.uid).once('value');
            if (snapshot.val()) {
...
export async function GetLocation() {
    try {
      ...
      const filteredLocations = Object.values(locations).filter(location => location !== currentUserUid);
